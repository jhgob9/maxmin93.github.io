...
public class Text {
  ...
  // 그럼에도 Set 으로 받은 이유는, 이왕이면 인터페이스로 받는게 일반적으로 좋은 습관이다
...
이때 어떤 프로그래머는 집합들을 배열 하나에 넣고 생애주기의 ordinal 값을 그 배열ㄹ의 인덱스로 사용하려 할 것이다.
...
잠시 여유를 갖고 정말 애너테이션으로 구현하는게 옳은지, 혹은 마커 인터페이스가 낫지는 않을지 곰곰히 생각해 보자
